...
x = Conv2D(64, (3, 3), strides=(2, 2), padding="same", kernel_regularizer=regularizers.l2(0.01))(main_input)
...
x = Conv2D(128, (3, 3), strides=(2, 2), padding="same", kernel_regularizer=regularizers.l2(0.01))(x)
x = BatchNormalization()(x)
x = Activation("relu")(x)
...
x = Conv2D(256, (3, 3), strides=(2, 2), padding="same", kernel_regularizer=regularizers.l2(0.01))(x)
x = BatchNormalization()(x)
x = Activation("relu")(x)
...
for i in range(start_from // save_every_n_epoch, n_epochs // save_every_n_epoch):
    ...
    model.save_weights("../weights/implementation8-bn-" + str(i * save_every_n_epoch) + ".h5")

    # save sample images
    whole_image_check_overlapping(model, 40, "imp8-bn-" + str(i * save_every_n_epoch) + "-")

    # save history
    output = open('../history/imp8-bn-{:0=4d}.pkl'.format(i * save_every_n_epoch), 'wb')

...
last = Conv2D(64, (3, 3), padding="same", activation="sigmoid", kernel_regularizer=regularizers.l2(0.01))(last)
last = Conv2D(64, (3, 3), padding="same", activation="sigmoid", kernel_regularizer=regularizers.l2(0.01))(last)

last = UpSampling2D(size=(2, 2))(last)
last = Conv2D(32, (3, 3), padding="same", activation="sigmoid", kernel_regularizer=regularizers.l2(0.01))(last)
last = Conv2D(2, (3, 3), padding="same", activation="sigmoid", kernel_regularizer=regularizers.l2(0.01))(last)
...
n_epochs = 10000
model.load_weights("../weights/implementation7d-reg-25.h5")
...
for i in range(start_from // save_every_n_epoch, n_epochs // save_every_n_epoch):
    ...
    model.save_weights("../weights/implementation7d-reg-" + str(i * save_every_n_epoch) + ".h5")

    # save sample images
    whole_image_check(model, 20, "imp7d-reg-" + str(i * save_every_n_epoch) + "-")

    # save history
    output = open('../history/imp7d-reg-{:0=4d}.pkl'.format(i * save_every_n_epoch), 'wb')

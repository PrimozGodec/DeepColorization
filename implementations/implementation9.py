...
from implementations.support_scripts.common import make_prediction_sample, make_prediction_sample_part, whole_image_check
...
def repeat_output(input):
    shape = K.shape(x)
    return K.reshape(K.repeat(input, 16 * 16), (shape[0], 16, 16, 256))
...
for i in range(start_from // save_every_n_epoch, n_epochs // save_every_n_epoch):
    ...
    # save sample images
    whole_image_check(model, 20, "imp9-" + str(i) + "-")

    # save history
    output = open('../history/imp9-{:0=4d}.pkl'.format(i), 'wb')

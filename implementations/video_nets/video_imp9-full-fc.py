...
from keras.layers import Conv2D, UpSampling2D, Lambda, Dense, Merge, merge, concatenate, regularizers, Add, add, \
    Conv2DTranspose, MaxPooling2D, Flatten, Reshape
...
# try to make dense and then reverse it
last = Flatten()(last)
last = Dense(100352, activation="relu")(last)


def reshape(x):
    K.reshape(x, (b_size, 28, 28, 128))

last = Lambda(reshape)(last)
...
g = video_imp9_full_generator(b_size, "../../data/video/training", num_neighbours=4, random=True)
gval = video_imp9_full_generator(b_size, "../../data/video/validation", num_neighbours=4, random=False)
...
for i in range(start_from // save_every_n_epoch, n_epochs // save_every_n_epoch):
    ...
    model.save_weights("../../weights/video-imp9-full-4-" + str(i * save_every_n_epoch) + ".h5")

    # save sample images
    video_visual_checker_imp9_full(model, 32, "video-imp9-full-4-" + str(i * save_every_n_epoch) + "-", num_neighbours=4)

    # save history
    output = open('../../history/video-imp9-full-4-{:0=4d}.pkl'.format(i * save_every_n_epoch), 'wb')

...
g = video_imp9_full_generator(b_size, "../../data/video/training", num_neighbours=3, random=True)
gval = video_imp9_full_generator(b_size, "../../data/video/validation", num_neighbours=3, random=False)
...
for i in range(start_from // save_every_n_epoch, n_epochs // save_every_n_epoch):
    ...
    model.save_weights("../../weights/video-imp10-full-3-" + str(i * save_every_n_epoch) + ".h5")

    # save sample images
    video_visual_checker_imp9_full(model, 32, "video-imp10-full-3-" + str(i * save_every_n_epoch) + "-", num_neighbours=3)

    # save history
    output = open('../../history/video-imp10-full-3-{:0=4d}.pkl'.format(i * save_every_n_epoch), 'wb')
